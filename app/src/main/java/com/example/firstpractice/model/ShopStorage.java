package com.example.firstpractice.model;

import com.example.firstpractice.R;

public class ShopStorage {
    public static final String textFirstShop = "That is the first shop, located on 11-ya Parkovaya 36";
    public static final int imageFirstShop = R.drawable.shop1;

    public static final String textSecondShop = "It's the second shop, it located on Saratovskoe Shosse 21";
    public static final int imageSecondShop = R.drawable.shop2;
}

